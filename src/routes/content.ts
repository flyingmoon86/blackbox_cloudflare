...
export type SiteProfileRow = { troupe_name: string; introduction: string | null; contact_email: string | null; contact_wechat: string | null; qq_group: string; public_account: string; recruitment: string; requirements: string; featured_production_id: number | null; page_texts: string };
...
contentRoutes.get("/announcements", async (c) => {
  const result = await c.env.DB.prepare("SELECT id,title,content,created_at FROM announcement ORDER BY created_at DESC,id DESC").all<AnnouncementRow>();
...
contentRoutes.get("/announcements/:id", async (c) => {
  const item = await c.env.DB.prepare("SELECT id,title,content,created_at FROM announcement WHERE id=?").bind(Number(c.req.param("id"))).first<AnnouncementRow>();
  return item ? c.html(announcementDetailPage(item, c.get("user")!.role === "admin", await csrfFor(c))) : c.text("公告不存在。", 404);
});

contentRoutes.get("/admin/announcements/new", async (c) => {
  const denied=adminDenied(c); if(denied)return denied;
  return c.html(announcementFormPage(await csrfFor(c)));
});

contentRoutes.post("/admin/announcements/new", async (c) => {
  const denied=adminDenied(c); if(denied)return denied; const form=await c.req.formData();
  ...
  const result=await c.env.DB.prepare("INSERT INTO announcement(title,content) VALUES(?,?)").bind(title,content).run();
...
contentRoutes.post("/admin/announcements/:id/delete", async (c) => {
  const denied=adminDenied(c); if(denied)return denied; const form=await c.req.formData();
  if(!csrfValid(c,form.get("csrf")))return c.text("请求已失效，请刷新页面后重试。",400);
  await c.env.DB.prepare("DELETE FROM announcement WHERE id=?").bind(Number(c.req.param("id"))).run();
...
contentRoutes.get("/admin/site", async (c) => {
  ...
  const productions=await c.env.DB.prepare("SELECT id,title,year FROM production ORDER BY year DESC,id DESC").all<{id:number;title:string;year:number|null}>();
  return c.html(siteSettingsPage(profile!,productions.results,await csrfFor(c),c.req.query("saved")==="1"));
});
...
contentRoutes.post("/admin/site", async (c) => {
  ...
  await c.env.DB.prepare(`UPDATE site_profile SET troupe_name=?,introduction=?,contact_email=?,contact_wechat=?,qq_group=?,public_account=?,recruitment=?,requirements=?,featured_production_id=?,page_texts=? WHERE id=1`)
    .bind(String(form.get("troupe_name")??"").trim().slice(0,100)||"话剧队",String(form.get("introduction")??"").trim()||null,String(form.get("contact_email")??"").trim()||null,String(form.get("contact_wechat")??"").trim()||null,String(form.get("qq_group")??"").trim().slice(0,50),String(form.get("public_account")??"").trim().slice(0,100),String(form.get("recruitment")??"").trim(),String(form.get("requirements")??"").trim(),featured,JSON.stringify(texts)).run();
