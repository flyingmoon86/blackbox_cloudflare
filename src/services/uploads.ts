...
export async function completeUpload(env: Bindings, id: string): Promise<{ resourceId: number; status: string }> {
  ...
  if (
    ...
    !(await env.DB.prepare(
      "SELECT 1 FROM production p JOIN user u ON u.id=? WHERE p.id=? AND (p.is_hidden=0 OR (u.role='admin' AND u.status='active'))",
    )
      .bind(task.user_id, task.production_id)
      .first())
  )
    throw new UploadError("上传任务不存在。", 404);
  ...
  try {
    ...
    await env.DB.batch([
      ...
      env.DB.prepare(
        `INSERT INTO resource(production_id,edition_id,status,uploader_id,title,res_type,description,filename,original_name,preview_filename,source_upload_task_id,content_fingerprint) SELECT t.production_id,t.edition_id,CASE WHEN u.status='active' AND (u.role='admin' OR (u.role='member' AND u.member_id IS NOT NULL AND ?=1 AND NOT EXISTS(SELECT 1 FROM resource d WHERE d.content_fingerprint=?))) THEN 'approved' ELSE 'pending' END,t.user_id,t.title,t.res_type,t.description,t.object_key,t.original_name,?,t.id,? FROM upload_task t JOIN user u ON u.id=t.user_id WHERE t.id=? AND t.lease_token=? AND u.status='active' AND u.role IN ('user','member','admin') AND (u.role='admin' OR t.production_id IS NULL OR EXISTS(SELECT 1 FROM production p WHERE p.id=t.production_id AND p.is_hidden=0)) AND NOT EXISTS(SELECT 1 FROM resource WHERE source_upload_task_id=t.id) ON CONFLICT DO NOTHING`,
