...
export function queueCurrentAvatar(env: Bindings, memberId: number): D1PreparedStatement {
  return env.DB.prepare(
    "INSERT INTO file_cleanup_task(object_key) SELECT photo FROM member WHERE id=? AND photo LIKE 'member-avatars/%' ON CONFLICT(object_key) DO UPDATE SET next_attempt_at=CURRENT_TIMESTAMP",
  ).bind(memberId);
}
...
export async function replaceAvatar(
  ...
  await reserveStorage(env, { id, bytes: data.byteLength, userId, kind: "avatar" });
  ...
  try {
    ...
    const result = await env.DB.batch([
      ...
      env.DB.prepare(
        "UPDATE member SET photo=? WHERE id=? AND EXISTS(SELECT 1 FROM user WHERE id=? AND member_id=? AND role='member' AND status='active')",
      ).bind(key, memberId, userId, memberId),
      ...storageCommitStatements(env, id, [{ key, size: data.byteLength }]),
      env.DB.prepare(
        "DELETE FROM file_cleanup_task WHERE object_key=? AND EXISTS(SELECT 1 FROM member WHERE id=? AND photo=?)",
      ).bind(key, memberId, key),
    ]);
    if (result[1].meta.changes !== 1) throw new HTTPException(409, { message: "账号与档案状态已变化，请刷新后重试。" });
  } catch (error) {
    await env.DB.prepare("UPDATE file_cleanup_task SET next_attempt_at=CURRENT_TIMESTAMP WHERE object_key=?")
      .bind(key)
      .run();
    throw error;
  } finally {
