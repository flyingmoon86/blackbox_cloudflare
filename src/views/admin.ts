...
export function adminDashboardPage(
  ...
  return layout(
    ...
    <section id="member-requests" class="admin-section"><div class="section-heading"><div><p class="eyebrow">MEMBER REQUESTS</p><h2>待审核队员申请</h2></div>${badge(counts.member_requests)}</div><div class="review-grid">${requestRows}</div></section><details class="admin-section admin-account-panel" id="accounts"><summary>账号管理 · ${users.length} 个账号</summary><div class="table-wrap"><table><thead><tr><th>用户名</th><th>身份</th><th>队员档案</th><th>状态</th><th>操作</th></tr></thead><tbody>${userRows}</tbody></table></div></details>`,
    true,
    true,
  );
}
