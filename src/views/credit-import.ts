...
export function importBatchPage(
  ...
  return shell(
    ...
    ${b.status === "ready" && !b.details_purged ? `<form class="import-confirm" method="post" action="${url}/confirm" data-import-confirm data-status-url="${url}/status">${token(csrf)}<input type="hidden" name="revision" value="${b.revision}"><label><input type="checkbox" name="confirm" value="yes" required>已核对版本、人员和角色，同意新增 ${b.add_count} 条关联及 ${b.create_member_count} 份最小档案</label><p class="hint">全有全无；若数据已变化，将停止提交并要求重新检查。</p><button>确认入库</button><div class="import-commit-progress" hidden><progress aria-label="导入提交状态"></progress><p role="status" aria-live="polite" data-import-message>正在提交整批数据，完成前无法显示逐行百分比。</p><a href="${url}" data-import-check hidden>打开批次页核对实际结果</a></div></form>` : ""}
