...
**状态（2026-09-13）：**认证提醒、首页正文空间修正、特别致谢、作品/队员排序及安全改进已发布到 Cloudflare。正式数据库已执行迁移 0001—0014；46 项后端回归、构建检查与 9 项线上冒烟通过。新增三个首次登录必须改密的管理员，原有作品、档案、资料保留。视频上传、邮箱自助找回继续关闭。实际版本及核对结果见验证记录。
...
Version 1.0 后续修正：PC 首页第二幕没有内部滚动，滚轮可直接切回第一幕，长介绍可弹窗展开；海报占满右侧可用高度。认证档案按年份倒序分组，横版作品采用更宽的两列布局。
...
4. 执行 `npm run db:production:migrate`，再 `npm run deploy`。本轮 0014 新增 `user.must_change_password`（默认 0，旧账号不变），应先迁移再发布引用新列的代码。新管理员必须在强制改密代码发布后创建，不能提前发放临时密码。

### 临时管理员与本轮安全/延迟改进

临时管理员设置 `must_change_password=1`。服务器只允许进入个人中心、修改密码、退出及加载主题；其他页面重定向、写操作返回 403。新密码不得与临时密码相同，改密增加 `auth_version`、清除会话并要求重新登录。生成的临时密码仅保存在本机被 Git 忽略的交付文件，不在数据库中保存明文，也不写入文档。

浏览器跨站写请求在读取请求体和数据库之前被拦截；原有 CSRF、账号鉴权和限流仍保留。CSP 明确仅允许本站脚本并禁用嵌入对象。主题 CSS 把背景资格和页面配色合并为一次查询；首屏背景跳过没有展示图的资料，避免失败请求。没有启用会泄露身份数据的整页公共缓存，实际大陆网络延迟仍需真实用户测量。
